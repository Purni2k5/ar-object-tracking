...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour, ISettings
	{
		...
		[Serializable]
		public class TransformationComplete : UnityEvent<Transform>
		{
			
		}
		[Serializable]
		public class UndoEvent : UnityEvent<Transform>
		{
		
		}
		[Serializable]
		public class RedoEvent : UnityEvent<Transform>
		{
			
		}

		public UndoEvent undoEvent = new UndoEvent();
		public RedoEvent redoEvent = new RedoEvent();

		public TransformationComplete transformationComplete = new TransformationComplete();
		...
		void HandleUndoRedo()
		{
			...
			if(Input.GetKey(ActionKey) | isUndo | isRedo)
			{
				if(Input.GetKeyDown(UndoAction) | isUndo)
				{
					UndoRedoManager.Undo();
					if (undoEvent != null)
					{
						undoEvent.Invoke(mainTargetRoot);
					}
				}
				else if(Input.GetKeyDown(RedoAction) | isRedo)
				{
					UndoRedoManager.Redo();
					if (redoEvent != null)
					{
						redoEvent.Invoke(mainTargetRoot);
					}
				}
			}
		}
...
//				AddTargetHighlightedRenderers(target);
		...
		public void SetPivotPoint()
		{
			if(mainTargetRoot != null)
			{
				...
				if (transformationComplete != null)
				{
					transformationComplete.Invoke(mainTargetRoot);
				}
			}
		}
